...
- **에픽세븐 비밀상점(비상런) 자동화, 창 크기 16:9 맞춤 및 마우스 드래그 모듈 완성 (v2.5.0)**:
  ...
  - **비상런 중복 구매 및 모달 대형 아이콘 오인식/배경 아이템 오클릭 원천 해결 (v2.5.2)**:
    - **원인 분석 (Root Causes)**:
      1) **모달 대형 아이콘 오인식**: 구매 확인 팝업 모달이 뜰 때 중앙에 나타나는 대형 아이콘(`cov_adb` 등)을 다중 스케일 매칭이 유효 아이템으로 판독.
      2) **상대 X 좌표 계산 오차**: 중앙 모달 아이콘(X ~ 0.50W) 기준 `+ 0.4718W`를 더해 X ~ 0.97W(화면 맨 우측)를 클릭하면서 뒤쪽 배경에 깔린 엉뚱한 일반 아이템(예: Row 1 조합 주문서 등)의 구매 버튼을 오클릭.
      3) **동일 사이클 내 중복 구매**: 1페이지에서 이미 구매한 아이템(예: 성약, 우정)이 2페이지 스크롤 후에도 화면 상단에 남아있을 때, 동일 사이클 내 재구매 시도로 인해 팝업/확인 루프가 꼬이면서 인접 일반 아이템 구매로 이어짐.
      4) **그레이스케일 매칭 한계**: 흑백 변환 시 일반 장비/주문서/골드 코인의 명암 구배가 성약/신비/우정과 유사하여 오인식 유발 (사용자 스크린샷 검증 결과 우정 클로버 템플릿이 회색조 시 골드 코인에 0.7926으로 오인식됨 확인).
    - **4중 방어선 해결 내역**:
      1) **3채널 BGR 컬러 매칭 전면 적용**: `cov.png`, `mys.png`, `fb.png` 원본 컬러 3채널로 `cv2.TM_CCOEFF_NORMED` 교차 검증 수행. 잡템/골드 코인 오인식 일치율 0%로 완전 배제 (진짜 우정 아이콘만 0.9599 칼같이 단독 검출).
      2) **오버사이즈/모달 전용 템플릿(`cov_adb`, `mys_adb`) 제거**: 실제 상점 행 규격 슬롯 템플릿만 메모리에 적재하여 모달 팝업의 거대 아이콘 매칭 원천 차단.
      3) **아이템 행 ROI 엄격 제한**: `0.10*W <= cx <= 0.55*W` 및 `0.05*H <= cy <= 0.95*H`로 가드를 두어 화면 중앙/우측의 모달 팝업 아이콘 감지 원천 배제.
      4) **동일 사이클 1회 구매 보장 (`bought_in_cycle: Set[str]`)**:
         - 한 번 구매한 아이템은 해당 사이클의 `bought_in_cycle`에 기록하여 재탐색 및 재클릭 원천 차단.
         - 1페이지에서 모든 목표 아이템을 샀으면 2페이지 스크롤을 생략하고 즉시 새로고침으로 직행.
         - 2페이지에서는 아직 구매하지 못한 목표 아이템만 선택적 탐색.
      5) **PC 클라이언트 구매 버튼 X좌표 절대화**: `BUY_BTN_X_RATIO = 0.8850`으로 고정하여 아이콘 X 오프셋 계산으로 엉뚱한 좌표를 클릭하는 위험 100% 제거.
      6) **팝업 잔상 해소 대기 (`delay_post_confirm = 0.50s`)**: 확인 클릭 후 팝업 모달이 완전히 페이드아웃될 때까지 대기.
    - **검증 완료**:
      - `test_suite.py`에 `test_secret_shop_duplicate_and_false_click_prevention` 추가, 전체 21개 단위 테스트 100% 통과 (`Ran 21 tests in 6.555s - OK`).
      - 실제 사용자 업로드 스크린샷 2종에 대한 픽셀 정밀 교차 검증 완료.
- **전체 화면 스크롤, 고정 시작 버튼 및 UI 초경량화 최적화 (v2.5.3)**:
  - **사용자 제기 문제**:
    1) 작은 해상도 디스플레이나 창을 축소했을 때 화면 스크롤이 지원되지 않아 하단 버튼(`▶ 매크로 시작` 등)이 잘려 창 크기를 수동으로 계속 키워야 하는 불편함.
    2) CustomTkinter 기본 스크롤의 자식 위젯(특히 `CTkSlider`) 휠 이벤트 무시 버그로 인해 버튼 목록 위에서 마우스 휠이 먹통이 됨.
    3) 매 프레임 UI 스레드에서 반복되는 OpenCV/PIL 캔버스 렌더링 및 고빈도 실시간 로그 기록 시 이벤트 큐 적체로 인한 UI 렉 및 무거움.
  - **해결 내역**:
    1) **하단 고정 제어 독 (Docked Control Bar)**:
       - 좌측 설정 패널(Column 1)을 `ctrl_box`(하단 고정)와 `self.col1_scroll`(상단 스크롤 프레임)의 2단 도킹 구조로 분리.
       - 창 높이가 아무리 작아져도 `▶ 매크로 시작` / `⏸ 일시정지` 버튼이 항상 화면 하단에 100% 고정 노출되어 스크롤 없이 즉시 조작 가능.
    2) **전역 쾌속 부드러운 스크롤 라우터 (`setup_smooth_scrolling`)**:
       - CustomTkinter의 `_check_if_valid_scroll`을 커스텀 패치하여 `CTkSlider` 차단 문제를 전면 해제.
       - 카드 내부의 모든 자식 위젯(슬라이더, 텍스트, 라벨, 이미지)에 `<MouseWheel>`을 재귀적으로 바인딩.
       - 기본 스크롤 휠 속도를 3배(`speed_multiplier=3`)로 상향하여 답답함 없는 시원한 스크롤 반응성 확보.
       - 좌측 설정 패널(`col1_scroll`), 버튼 목록(`targets_scroll`), 비상런 대시보드(`secret_shop_container`) 전 구역에 일괄 적용.
    3) **⚡ 초경량 모드 (실시간 미리보기 ON/OFF 토글 버튼)**:
       - 미리보기 화면 우측 상단에 "⚡ 초경량 모드 (미리보기 끄기)" 토글 버튼 탑재.
       - 초경량 모드 활성화 시 캔버스 OpenCV 리사이즈 + PIL 객체 변환을 100% 바이패스하여 CPU 점유율을 0%대로 낮추고 UI 반응 속도 극대화.
       - 일반 모드에서도 15 FPS 스로틀링(약 60ms 간격)을 적용하여 불필요한 고빈도 GUI 렌더링 방지.
    4) **50ms 비동기 로그 배치 플러시 (`_flush_logs`)**:
       - `append_log`에서 매 줄마다 `after(0)`을 호출하던 구조를 50ms 버퍼 큐 배치 플러시로 전환.
       - 대량 로그 발생 시에도 Tkinter 이벤트 루프 프리징 완벽 차단.
    5) **반응형 창 크기 최적화**:
       - 기본 창 크기 `1200x750`, 최소 지원 해상도 `980x560`으로 설정하여 FHD, QHD, 고DPI(125%/150%) 환경에서도 창 잘림 없는 최적의 사용성 제공.
    6) **무결성 및 안정성 검증**:
       - `test_suite.py` 21개 단위 테스트 100% 통과 (`ALL PASS`).
       - 기존 9개 버튼 프로필 (`profiles/default.json`) 100% 무손실 유지.
       - Tkinter GUI 초기화 및 렌더링 스모크 테스트 무결성 확인.
